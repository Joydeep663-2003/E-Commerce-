...
const seedDatabase = async () => {
  try {
    ...
    if (!mongoUrl) {
      console.log('❌ MONGO_URL missing in environment');
      return;
    }
    ...
    // Clear and Seed Categories Collection
    await Category.deleteMany({});
    const categoryDocs = categories.map(name => ({ name }));
    await Category.insertMany(categoryDocs);
    console.log('🏷️ Categories collection populated.');

    // Clear and Seed Products Collection
    ...
    categories.forEach((category) => {
      ...
      templates.forEach((tmplName, idx) => {
        ...
        list.push({
          ...
          price: basePrice,
          originalPrice,
          description: `Experience exceptional quality with ${title}. Designed with premium craftsmanship, durability, and top-of-the-line performance for daily excellence. Includes 1-Year Warranty & Free Delivery across India.`,
    ...
    console.log(`✅ Successfully seeded ${list.length} products (exactly 20 per category) into MongoDB!`);
    process.exit(0);
  } catch (err) {
